...
struct teamscore
{
    ...
    void addplayer(playerent *d)
    {
        ...
        deaths += d->deaths;
        points += d->points;
        if(m_flags || m_domination) flagscore += d->flagscore;
    }
    ...
    void addscore(discscore &d)
    {
        ...
        deaths += d.deaths;
        points += d.points;
        if(m_flags || m_domination) flagscore += d.flags;
    }
};
...
void renderdiscscores(int team)
{
    loopv(discscores) if(team == team_group(discscores[i].team))
    {
        ...
        if(m_flags || m_domination) line.addcol(sc_flags, "%d", d.flags);
...
void renderscore(playerent *d)
{
    ...
    if(m_flags || m_domination) line.addcol(sc_flags, "%d", d->flagscore);
    line.addcol(sc_frags, "%d", d->frags);
    line.addcol(sc_deaths, "%d", d->deaths);
    line.addcol(sc_ratio, "%.2f", SCORERATIO(d->frags, d->deaths));
...
int renderteamscore(teamscore *t)
{
    ...
    if(m_flags || m_domination) line.addcol(sc_flags, "%d", t->flagscore);
...
void reorderscorecolumns()
{
    ...
    if(m_flags || m_domination) sscore.addcol(sc_flags, "flags");
...
void renderscores(void *menu, bool init)
{
    ...
    { // ffa mode
        ...
        if(scores.length() > 0)
        {
            ...
            if(scores.length() > 1
                && (((m_flags || m_domination) && scores[0]->flagscore == scores[1]->flagscore)
                     || (!m_flags && !m_domination && scores[0]->frags == scores[1]->frags)))
                winner = -1;
        }
    }
    if(spectators)
...
const char *asciiscores(bool destjpg)
{
    ...
    {
        formatstring(text)("\n%sfrags deaths cn%s name\n", m_flags || m_domination ? "flags " : "", m_teammode ? " team" : "");
        addstr(buf, text);
    }
    loopv(scores)
    {
...
//         const char *sr = scoreratio(d->frags, d->deaths);
        ...
        if(destjpg)
            formatstring(text)("%s%s (%s%d/%d)\n", d->name, m_teammode ? team : "", m_flags || m_domination  ? flags : "", d->frags, d->deaths);
        else
            formatstring(text)("%s %4d   %4d %2d%s %s%s\n", m_flags || m_domination ? flags : "", d->frags, d->deaths, d->clientnum,
...
//         const char *sr = scoreratio(d.frags, d.deaths);
        ...
        if(destjpg)
            ...
            formatstring(text)("%s %4d   %4d --%s %s(disconnected)\n", m_flags || m_domination ? flags : "", d.frags, d.deaths, m_teammode ? team : "", d.name);
        addstr(buf, text);
    }
    if(destjpg)
    {
        extern int minutesremaining;
        formatstring(text)("(%sfrags/deaths), %d minute%s remaining\n", m_flags || m_domination ? "flags/" : "", minutesremaining, minutesremaining == 1 ? "" : "s");
        addstr(buf, text);
    }
    return buf;
}
