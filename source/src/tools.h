...
#define DEBUGS(v) if (DEBUGCOND) { v; }
#else
#define DEBUG(v) {}
#define DEBUGVAR(v) {}
#define DEBUGS(v) {}
...
extern const char *findfile(const char *filename, const char *mode, bool allow_outside_game = false);
...
extern stream *openfile(const char *filename, const char *mode, bool allow_outside_game = false);
extern stream *opentempfile(const char *filename, const char *mode);
extern stream *opengzfile(const char *filename, const char *mode, stream *file = NULL, int level = Z_BEST_COMPRESSION, bool allow_outside_game = false);
